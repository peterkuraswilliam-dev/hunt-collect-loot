...
export type LootSourceType = {
  ...
  description: string | null; sort_order: number; enabled: boolean; is_system: boolean;
};
export type LootTable = {
  id: string; slug: string; internal_id: string | null; name: string; description: string | null;
  category: string | null; source_type_id: string | null; enabled: boolean; tags: string[];
  min_rewards: number; max_rewards: number; allow_duplicates: boolean; guaranteed_first: boolean;
  weighted_random: boolean; total_rolls: number; total_rewards_granted: number;
  created_at: string; updated_at: string;
};
export type LootTableEntry = {
  id: string; loot_table_id: string; reward_id: string; weight: number; drop_chance: number;
  min_quantity: number; max_quantity: number; guaranteed: boolean; enabled: boolean;
  display_order: number; times_awarded: number;
};
export type LootTableReference = {
  id: string; loot_table_id: string; module: string; record_name: string; record_ref: string | null;
  status: string; last_updated: string;
};
export type LootTableActivityRow = {
  id: string; loot_table_id: string; action: string; actor_id: string | null;
  actor_label: string | null; detail: Record<string, unknown>; created_at: string;
};

export const lootSourceTypesQuery = queryOptions({
  queryKey: ["loot_source_types"],
  queryFn: async (): Promise<LootSourceType[]> => {
...
export const lootTablesQuery = queryOptions({
...
export const lootTableEntriesQuery = (id: string | null) =>
  queryOptions({
    queryKey: ["loot_table_entries", id],
    enabled: !!id,
    queryFn: async (): Promise<LootTableEntry[]> => {
      if (!id) return [];
      const { data, error } = await sb.from("loot_table_entries").select("*").eq("loot_table_id", id).order("display_order");
      if (error) throw error;
      return data ?? [];
    },
    staleTime: 15_000,
...
export const lootTableEntriesAllQuery = queryOptions({
  queryKey: ["loot_table_entries_all"],
  queryFn: async (): Promise<Array<{ loot_table_id: string; reward_id: string; weight: number; times_awarded: number }>> => {
    const { data, error } = await sb.from("loot_table_entries").select("loot_table_id,reward_id,weight,times_awarded");
    if (error) throw error;
    return data ?? [];
  },
  staleTime: 30_000,
...
export const lootTableReferencesQuery = (id: string | null) =>
  queryOptions({
    queryKey: ["loot_table_references", id],
    enabled: !!id,
    queryFn: async (): Promise<LootTableReference[]> => {
      if (!id) return [];
      const { data, error } = await sb.from("loot_table_references").select("*").eq("loot_table_id", id).order("last_updated", { ascending: false });
      if (error) throw error;
      return data ?? [];
    },
    staleTime: 30_000,
...
export const lootTableActivityQuery = (id: string | null) =>
  queryOptions({
    queryKey: ["loot_table_activity", id],
    enabled: !!id,
    queryFn: async (): Promise<LootTableActivityRow[]> => {
      if (!id) return [];
      const { data, error } = await sb.from("loot_table_activity_log").select("*").eq("loot_table_id", id).order("created_at", { ascending: false }).limit(100);
      if (error) throw error;
      return data ?? [];
    },
    staleTime: 15_000,
