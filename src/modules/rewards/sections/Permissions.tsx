...
export function Permissions() {
  ...
  const { data: perms = [] } = useQuery(modulePermissionsQuery("rewards"));
  ...
  const toggle = useMutation({
    mutationFn: async ({ role, cap, on }: { role: string; cap: string; on: boolean }) => {
      if (on) {
        const { error } = await sb.from("module_permissions").insert({ module: "rewards", role, capability: cap });
        if (error) throw error;
      } else {
        const { error } = await sb.from("module_permissions").delete().eq("module", "rewards").eq("role", role).eq("capability", cap);
        if (error) throw error;
      }
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["module_permissions", "rewards"] }),
  });
